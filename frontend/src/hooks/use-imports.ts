...
/** 前年実績(製品SC原価フロー) アップロード。prior-year-actuals キャッシュを invalidate。 */
export function useUploadPriorYearActuals() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: {
      file: File;
      fiscal_year: number;
      delete_existing?: boolean;
    }) => importsApi.uploadPriorYearActuals(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["import-batches"] });
      queryClient.invalidateQueries({ queryKey: ["prior-year-actuals"] });
    },
  });
}
...
/** 前年実績(仕掛品SC原価フロー) アップロード。prior-year-wip キャッシュを invalidate。 */
export function useUploadPriorYearWip() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: {
      file: File;
      fiscal_year: number;
      delete_existing?: boolean;
    }) => importsApi.uploadPriorYearWip(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["import-batches"] });
      queryClient.invalidateQueries({ queryKey: ["prior-year-wip"] });
    },
  });
}
